...
public class KarzaConnector implements CustomerNetworkDao {
    ...
    public Maybe<HttpResponse<KarzaResponseData>> getPanStatusFromKarzaUsingMaybe(Customer customer) {
        ...
        return Maybe.fromCallable(() -> httpClient.toBlocking().exchange(request, KarzaResponseData.class))
                .subscribeOn(Schedulers.io())
                .map(response -> (HttpResponse<KarzaResponseData>) response)
                .doOnError(throwable -> logger.severe("Error calling third-party service: " + throwable.getMessage()))
                .onErrorComplete();
    }
}
